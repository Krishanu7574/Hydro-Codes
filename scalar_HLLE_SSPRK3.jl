...
uana = zeros(Float64,length(u))
uana .= (x_list .- a*tf .≥ -0.4) .& (x_list .- a*tf .≤ -0.1)
...
ax1 = Axis(fig[1, 1], xlabel = "x", ylabel = "u", title = "1D Advection: HLLE + slope limiter + SSPRK3 N = $N")
...
lines!(ax1, x_list, uana, label="analytical", linestyle=:dot, color = :black)
axislegend(ax1; position = :lt)
ax2 = Axis(fig[1, 2], xlabel = "x", ylabel = "Slope", title = "Slope of u_final")
lines!(ax2, x_list, slopes_superbee(u), label = "Slope of u_final", color = :green)
