...
function initial_condition(x)
    if x < L_mid
        ...
        p =  0.1   # Pressure on the right (1e-6)*(2/3) , 0.1 , 995
    end
...
function relativistic_hlle_flux(U_L,U_R)
   ...
   c_s_L_2 = max(0,(γ*(γ - 1)*(e_L - n_L))/(n_L + γ*(e_L - n_L)))  # Given in paper (also derived)
   c_s_R_2 = max(0,(γ*(γ - 1)*(e_R - n_R))/(n_R + γ*(e_R - n_R)))  # Given in paper (also derived)
