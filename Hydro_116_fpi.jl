...
function initial_condition(x)
    if x < L_mid
        ρ =  2.0  # Rest-mass density on the left  1.0, 10.0,2.0
        v =  0.0  # Velocity on the left
        p =  1990.0 #13*(1/3)  # Pressure on the left   1.0, 40.0/3 ,1990
    else
        ρ =  1.0  # Rest-mass density on the right 1.0,0.125,1.0
        v =  0.0   # Velocity on the right
        p =  995.0
        #(2/3)*(10^-6)   # Pressure on the right (1e-6)*(2/3) , 0.1 , 995
    end
