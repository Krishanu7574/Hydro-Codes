...
function relativistic_hlle_flux(U_L, U_R)
    ...
    c_s_M_2 = (sqrt(U_L[3])*c_s_L_2 + sqrt(U_R[3])*c_s_R_2) / (sqrt(U_L[3]) + sqrt(U_R[3])) +
              ((v_R - v_L)^2)*((γ - 1)/2) * (sqrt(U_L[3] * U_R[3])/(U_R[3] + U_R[3])^2)
    c_s_M = sqrt(c_s_M_2)
...
D_vals = [U[i][1] for i in 1:N]
v_vals = [pressure_nu(U[i])[2] for i in 1:N]
p_vals = [pressure_nu(U[i])[1] for i in 1:N]

fig = Figure(size = (1200, 400))
ax1 = Axis(fig[1, 1], xlabel="x", ylabel="D", title="Density")
ax2 = Axis(fig[1, 2], xlabel = "x", ylabel = "v", title = "Velocity" , limits = ((0, 1), (-1, 1))) 
ax3 = Axis(fig[1, 3], xlabel="x", ylabel="p", title="Pressure")

plt1 = scatter!(ax1, x[1:2:end], D_vals[1:2:end], color=:green)
plt2 = scatter!(ax2, x[1:2:end], v_vals[1:2:end], color=:red)
plt3 = scatter!(ax3, x[1:2:end], p_vals[1:2:end], color=:blue)

# --- Animation recording ---
record(fig, "relativistic_HLLE_animation.mp4", 1:round(Int, T/dt/20)) do frame
    for _ in 1:20
    ...
    plt1[1][] = Point2f.(x[1:2:end], D_vals[1:2:end])
    plt2[1][] = Point2f.(x[1:2:end], v_vals[1:2:end])
    plt3[1][] = Point2f.(x[1:2:end], p_vals[1:2:end])
end
